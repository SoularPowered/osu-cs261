...
struct Node *_addNode(struct Node *cur, TYPE val)
{
	...
	if (cur == NULL) {
		...
		assert(newNode != 0);
		newNode->val = val;
		newNode->left = newNode->right = 0; // make new node's childs both null
		return newNode;
	}
	// if value is less than current node value, move left
	if (compare(val, cur->val) < 0) {
		cur->left = _addNode(cur->left, val);
	}
	// else if value is greater than or equal to current node value, move right
	else if (compare(val, cur->val) >= 0) {
		cur->right = _addNode(cur->right, val);
	}

	// Return the built tree to caller (public addBSTree in this case)
	return cur;
}
