...
void addHeap(DynArr *heap, TYPE val, comparator  compare)
{
  /* FIXME: Write This */
	...
	if (heap->size >= heap->capacity) {
	  _dynArrSetCapacity(heap, heap->size * 2);
	}

	heap->data[heap->size] = val;
	heap->size++;

	// Set indices of the newly added value and its parent
	int newValueIndex = heap->size - 1;    // newValueIndex starts as last position	
...
void _buildHeap(DynArr *heap, comparator compare)
{
  /* FIXME: Write This */
	assert(heap != NULL);
	/* the "heap" passed in starts unsorted. To sort it, we just repeatedly call
	_adjustHeap() on the indices start at heap->size and going down until index 0 	*/
	int max = heap->size;
	int i; // loop counter
	for (i = max/2 - 1; i >= 0; i--) {
...
void sortHeap(DynArr *heap, comparator compare)
{
  /* FIXME: Write this */
	...
	int i; // loop counter
	// Works by repeatedly putting the smallest value of minHeap at i, which decreases,
	// effectively placing the array in reverse sorted order using in-place memory storage
